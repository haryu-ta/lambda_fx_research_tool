...
1. **Given** 新しい為替APIの認証情報が有効である, **When** スケジュール実行が開始される, **Then** USD/JPYを取得し「HH:MM時点  1ドル = XXX.XX 円」で通知する。
...
- **FR-002**: システムは、新APIから取得したレートを既存成功通知フォーマット「HH:MM時点  1ドル = XXX.XX 円」で配信しなければならない。
...
- 成功通知の100%が文言フォーマット「HH:MM時点  1ドル = XXX.XX 円」に一致する
